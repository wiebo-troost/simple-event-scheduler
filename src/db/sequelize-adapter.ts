 ...
 * SequelizeAdapter is constructed as follows:
 *
 * const sequelizeAdapter = new SequelizeAdapter(sequelize, options);
 * You'll need to pass in your sequelize instance, and an optional SequelizeAdapterOptions object.
 * It has a single property 'tablename' whene you have override the default
 * name of 'job'.
 */
class SequelizeAdapter extends DBAdapter {
  ...
  public createJob(job: Job): Promise<Job> {
    ...
    return JobsModel.create(job).then((jobModel) => {
  ...
  public purgeJobs(query: any): Promise<number> {
    // eslint-disable-line
    return JobsModel.destroy({ where: query }).then((result) => {
      return result;
    });
  }
  ...
  public loadJobs(loadIntervalSeconds: number): Promise<Job[]> {
    ...
    return JobsModel.findAll(qry).then((models) => {
